...
//
// What leaves the device: a random device id, the site code, which feature was
// used, and when. Nothing a clinician typed — no scores, no patient details, no
// free text. The worker enforces the same list again on arrival, so widening
// what is collected takes a deliberate change in two places.
//
...
/**
 * Start collecting for a cohort. Called once the access code is known.
 * Without a cohort there is nothing to attribute an event to, so record() stays
 * inert until this runs.
 */
export function startMetrics(cohort, version) {
    cohortId = cohort;
    appVersion = version;
...
export async function flush({ keepalive = false } = {}) {
    if (!ENDPOINT || !cohortId || flushing || !navigator.onLine) return;
    ...
    try {
        ...
        if (response.ok) {
            ...
            writeQueue(current.filter(e => !sent.has(e.eid)));
                   ...
                   response.status !== 429) {
            // The server will never accept this batch — an unknown cohort, or
